...
```js
{
  ...
  tab:   '8/26 三',                  // 上方分頁按鈕文字
  date:  '2026 / 08 / 26（三）',      // 標題列日期
  title: '塔龍加動物園・Balmoral 海灘', // 當日標題
  chips: ['F2 渡輪＝迷你港灣遊船', ...],// 標題下的小標籤
  stops: [ ...站點 ... ],            // 見下
  legs:  [ ...路段 ... ],            // 見下
  note:  '底部黃色 NOTE 區的文字'      // 可省略
}
```
...
```js
{
  ...
  n:  '塔龍加動物園',                 // 名稱
  d:  '接駁上山由上往下逛…',           // 說明（可空字串）
  ll: [-33.8433, 151.2411],          // [緯度, 經度] → 決定地圖上編號位置
  q:  'Taronga Zoo Sydney'           // Google Maps 導航用的搜尋字串
}
```
...
- **改某天某站的時間／名稱／說明**：找到對應 `DAYS` 裡那天的 `stops`，改 `t`/`n`/`d`。
- **新增一個站點**：在該天 `stops` 插入一個站點物件，**並且**在 `legs` 對應位置補一段（維持 `legs.length === stops.length - 1`），否則地圖線會對不上。記得給新站點正確的 `ll` 座標和 `q` 搜尋字串。
- **刪除一個站點**：刪 `stops` 那筆，**同時**刪掉相鄰的一段 `legs`。
- **改交通方式**：改該段 `legs` 的 `m`（要用 MODES 既有的 key）。
...
- 改完可用 `node --check`（把 `<script>` 內容抽出）或直接開瀏覽器 Console 看有無錯誤。
