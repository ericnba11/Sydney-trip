...
- Firestore → Rules 開放 `notes` 這個 collection：
  ```
  rules_version = '2';
  ...
  ```
- 如果 `firebaseConfig` 還是佔位值（`PASTE_YOUR_...`），會自動降級成「只存在瀏覽這台裝置的 localStorage」，不會報錯、UI 一樣能用。
- 這些 `firebaseConfig` 值是**公開識別碼不是密碼**，可以放心留在公開的 GitHub repo 裡；真正的存取控制在 Firestore Rules。因為沒有登入機制，**任何拿到網址的人理論上都能改備註**，風險等同於一份公開可編輯的雲端文件，適合這種私人小型旅遊網站，但不要拿來存敏感資訊。
- 資料結構：一份文件對應一個站點，doc id 是 `${dayId}_${stopIndex}`（例如 `d4_3`），內容 `{text, updatedAt}`（沒有 author）。

---

## 6.6 網頁內建編輯模式（已移除，2026-07-19）

曾經做過一版「header 按鈕切換唯讀／編輯，站點時間/名稱/說明/時長、路段說明都能直接在網頁上改，存進另一個 Firestore collection `edits`」的功能，但上線後在 GitHub Pages 正式環境發生**所有站點名稱／說明都被清空變白**的 bug（本機 `file://` 測試沒重現，懷疑跟 Firestore `edits` collection 的 `onSnapshot` 即時同步在正式網域環境下的行為有關，確切成因沒有查到底），使用者要求先整個移除，**之後如果要重新做手動編輯功能再說**。

目前 `index.html` 已經把這個功能相關的程式碼（`EDIT_MODE`、`editsCache`、`fieldKey`／`resolveField`／`saveEdit`／`refreshEditableFields`、`edits` 的 Firestore 監聽、header 的「編輯行程」按鈕、以及 `buildAllDays()` 裡的 `EDIT_MODE` 分支）**全部拿掉**，`notes` 那個 collection／備註功能不受影響、正常運作。

若之後要重做，這裡有幾個教訓可以參考：
- 不要在「唯讀畫面」和「Firestore 即時同步監聽」之間留有隱性耦合——上一版的 bug 很可能跟 `onSnapshot` 回呼直接改寫 DOM 文字內容、又依賴 `dataset.default`／`_defaultText` 這種存在 DOM 節點上的快取值有關，這類設計在正式環境的網路時序（連線延遲、重連、多次觸發）下比本機測試複雜很多，**務必在真正部署的網址上測試，不能只信任本機 `file://` 測試結果**。
- 比較穩妥的做法可能是：唯讀畫面完全不含任何 `data-editkey` 之類的即時同步掛勾，只有「進入編輯模式」那個當下才去讀一次 Firestore（用 `.get()` 而不是 `.onSnapshot()`），改完就離開編輯模式、丟棄監聽，降低背景同步邏輯意外覆寫畫面的風險。
