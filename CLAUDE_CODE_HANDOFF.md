...
每個站點下方有一個「備註」面板，任何看這個網頁的人都可以留言，**設定好 Firebase 後會即時同步**——一人存檔，其他人重新打開面板就看得到。

- 後端用 **Firebase Firestore**（免費、免伺服器，跟 GitHub Pages 這種純靜態網站相容）。
- 程式碼裡的 `firebaseConfig`（在 `<script>` 前段）目前是**佔位值**（`PASTE_YOUR_...`）。只要還沒填入真實設定，功能會自動降級成「只存在瀏覽這台裝置的 localStorage」，不會報錯、UI 一樣能用。
- 要接上真的同步，使用者需要：
  1. 到 https://console.firebase.google.com/ 建立一個免費專案（不需信用卡）。
  2. Build → Firestore Database → Create database → 先選 production/test 模式都可以，之後用下面的規則。
  3. Project settings → 一般 → 新增 Web App，複製 `firebaseConfig` 物件（`apiKey`／`authDomain`／`projectId`／`storageBucket`／`messagingSenderId`／`appId`）貼進程式碼取代佔位值。
  4. Firestore → Rules 貼上（只開放 `notes` 這個 collection，範圍夠小）：
     ```
     rules_version = '2';
...
- 資料結構：一份文件對應一個站點，doc id 是 `${dayId}_${stopIndex}`（例如 `d4_3`），內容 `{text, author, updatedAt}`。
